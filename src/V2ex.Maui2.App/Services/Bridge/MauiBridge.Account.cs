...
public partial class MauiBridge
{
    ...
    public async Task<string> GetSignInPageInfoAsync()
    {
        try
        {
            ...
            var imageData = await apiService.GetCaptchaImage(formInfo);
            var base64Image = Convert.ToBase64String(imageData);
            return JsonSerializer.Serialize(new
            {
                success = true,
                usernameFieldName = formInfo.NameParameter,
                passwordFieldName = formInfo.PasswordParameter,
                captchaFieldName = formInfo.CaptchaParameter,
                once = formInfo.Once,
    ...
    public async Task<string> GetCaptchaImageAsync(string once)
    {
        try
        {
             ...
             if (formInfo.Once != once)
             {
                 ...
                 // ideally we'd just use formInfo.Captcha url
             }
