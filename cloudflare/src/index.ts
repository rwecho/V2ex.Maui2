...
async function handleRegister(request: Request, env: Env): Promise<Response> {
    ...
    try {
        ...
        if (!feedUrl.includes("v2ex.com/feed")) {
    ...
    const results = await Promise.allSettled(keys.map(async (key) => {
        ...
        for (const item of newItems) {
            const success = await sendPushNotification(
                 fcmToken,
                 item.title, 
                 newItems.length > 1 ? item.content.substring(0, 100) + "..." : item.content,
                 { 
                     link: item.link, 
...
async function checkHotTopics(env: Env, history: any[]) {
    try {
        ...
        const processedStr = await env.V2EX_PUSH_KV.get("global:processed_hot_topics");
        const processedIds: number[] = processedStr ? JSON.parse(processedStr) : [];
        
        const newHotTopics = hotTopics.filter((t: any) => !processedIds.includes(t.id));
        ...
        await env.V2EX_PUSH_KV.put("global:processed_hot_topics", JSON.stringify(processedIds));
...
async function getAllUserKeys(env: Env): Promise<any[]> {
    ...
    do {
        const list: { keys: any[]; list_complete: boolean; cursor?: string } = await env.V2EX_PUSH_KV.list({ prefix: "user:", cursor });
